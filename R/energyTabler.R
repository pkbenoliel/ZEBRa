...
#'
#' @return 0 if the table was created successfully (or already exists)
#' @export
#'
#' @examples
#'
energy_tabler <- function(gtfs.obj, large.Ag, mass.vec = NULL, mass.labels = NULL, input.loc = "./GTFS Local Data/GMapsResults",
                          output.loc = "./Energy Tables", summary.loc = "./Results/Pre Fuel", ignore.Existing = FALSE) {
  ag.name <- gtfs.obj$agency$agency_name
  if(!ignore.Existing & file.exists(file.path(input.loc, paste(ag.name, "Energy Table.csv")))) {
