...
ZEBRaLite <- function() {
  ...
  ui <- fluidPage(
    ...
    tabsetPanel(
      ...
      tabPanel("Dashboard",
               column(6,
                      ...
                      bsTooltip("oppChargers", "Enter the number of chargers to be used.  Only active if the mode of 'Provide a charger to each route' is not selected.",
                      ...
                      bsTooltip("operatingYears", "Determines how long the buses are planned to be used in service.  This affects the overall impact of the energy and maintenance costs on the overall system cost.",
                                "right", options = list(container = "body")),
                      bsTooltip("diesel", "The cost per gallong of diesel fuel.  Used to estimate the overall savings of switching to BEBs over the lifetime of the system.",
                                "right", options = list(container = "body")),
                      bsTooltip("mapButton", "Draws the map of the network, including identified charging locations. Requires that a GTFS feed be uploaded.",
                                "right", options = list(container = "body")),
                      bsTooltip("goButton", "Performs the main cost calculation.  Requires that the 'Map' button has been presesed beforehand.",
                                "right", options = list(container = "body"))
                      ),
                 column(6,
                        shinySaveButton("saveButton", "Save Current Scenario", "Save as...", filetype = list(rds = "rds")),
                        shinyFilesButton("loadButton", "Load Saved Scenario", "Load...", multiple = FALSE, buttonType = "default", class = NULL),
    ...
    #                                   "Serving the most frequently run routes" = "mode4"
                        ...
                        bsTooltip("dutyRatio", "A percentage representing approximately how much of the operating hours that the chargers will have a bus actively attached and charging.",
                                  "right", options = list(container = "body"))
                        )
        ),
        tabPanel("Results",
  ...
  ## ----------------------|
    ...
    server <- function(input, output, session) {
      ...
      saveloadPaths <- reactiveValues(savePath = NULL, loadPath = NULL)
      ...
      observeEvent(input$mapButton, {
        ...
        cand.locs.tmp <- cand_loc_mode(gtfsObj, candLocMode(), input$oppChargers)
      ...
      observe({
        shinyFileSave(input, "saveButton", roots = roots, session = session)
      })

      observeEvent(input$saveButton, {
        fileinfo <- parseSavePath(roots, input$saveButton)
        if (nrow(fileinfo) > 0) {
          saveloadPaths$savePath <- fileinfo$datapath
        }
      })

      observeEvent(saveloadPaths$savePath, {
        if (!is.null(saveloadPaths$savePath)) {
          saveObject <- list("BusTable" = virtBusTable$data, "TimeTable" = virtTimeTable$data, "Map" = cand.map$data, "Table" = cand.table$data, "EnergyTable" = virtEnergyChoices$data)
          saveRDS(saveObject, saveloadPaths$savePath)
          showNotification("Saved", type = "message")
          saveloadPaths$savePath <- NULL  # Reset the save path
        }
      })
      ...
      observe({
        shinyFileChoose(input, "loadButton", roots = roots, session = session)
      })

      observeEvent(input$loadButton, {
        fileinfo <- parseFilePaths(roots, input$loadButton)
        if (nrow(fileinfo) > 0) {
          saveloadPaths$loadPath <- fileinfo$datapath[1]
        }
      })

      observeEvent(saveloadPaths$loadPath, {
        if (!is.null(saveloadPaths$loadPath)) {
          exampleData(FALSE)
          loadedScenario <- readRDS(saveloadPaths$loadPath)
          showNotification("Scenario loaded successfully.", type = "message")
          saveloadPaths$loadPath <- NULL  # Reset the load path
        }

        virtBusTable$data <- loadedScenario$BusTable$data
        virtTimeTable$data <- loadedScenario$TimeTable$data
        cand.map$data <- loadedScenario$Map$data
        cand.table$data <- loadedScenario$Table$data
        virtEnergyChoices$data <- loadedScenario$EnergyTable$data
